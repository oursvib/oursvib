...
<form action="javascript:void(0)" method="post" id="blockbooking">
    <div class="modal-header">
        <h4 class="modal-title">Edit Blocked dates </h4>
    ...
    <div class="modal-body">
        ...
        <div class="utf-no-border margin-bottom-20">
            ...
            <select class="form-control" name="listing" id="listing">
                ...
                <option value="{{$listing->id}}" @if($listing->id==$bookings->listing_id) selected="selected" @endif >{{$listing->title}}</option>
                <?php } ?>
            </select>
        ...
        <div class="utf-no-border">
            ...
            <div class="input-append date" id="datetimepickerfrom" data-date="{{ date('d-m-Y H:i')}}" data-date-format="dd-mm-yyyy hh:ii">
                <input class="form-control" name="datetimepickerfrom" size="16" type="text" value="{{$bookings->start_date}}" >
                <span class="add-on"><i class="icon-th"></i></span>
            </div>
            <p>Date To</p>
            <div class="input-append date" id="datetimepickerto" data-date="{{ date('d-m-Y H:i')}}" data-date-format="dd-mm-yyyy hh:ii">
                <input class="form-control" name="datetimepickerto"  size="16" type="text" value="{{$bookings->end_date}}" >
                <span class="add-on"><i class="icon-th"></i></span>
            </div>
        </div>
...
<script>
    ...
    $("#blockbooking").validate({
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                url: "{{url('/vendors/updateblockbooking')}}" ,
                type: "POST",
